...
def receive_and_upload(host='127.0.0.4', port=65524):
    load_dotenv()
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        mnemonic = "baby life speak lounge wrestle demise human length high smile switch bread draft there jewel icon famous piano tape syrup frost march seven kiss"
...
def store_and_measure_time(private_key, vehicle_data, counter):
    tag = "0x" + f"vehicle3_data#{counter}".encode("utf-8").hex()
    ...
    # local node
    client = Client(nodes=['http://localhost:80']) 
